...
export class EsaApi {
  ...
  constructor({
    ...
    this.team = team
    this.client = axios.create({ baseURL: 'https://api.esa.io/v1' })
    this.client.interceptors.request.use(enheadAuthorization(token))
    this.client.interceptors.response.use(embodyRatelimit)
    this.client.interceptors.response.use(embodyTeamName(team))
  }
